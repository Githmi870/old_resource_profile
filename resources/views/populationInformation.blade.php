<x-app-layout>
    @vite('resources/js/pages/pop_con.js')
    <div class="py-6">

        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 mb-5">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h2 class="text-xl font-semibold mb-4 dark:text-gray-200">1 වයස් කාණ්ඩ අනුව ජනගහනය </h2>

                <table class="shadow-md border rounded-lg w-full mb-5" border="1">
                    <tr>
                        <th></th>
                        <th class="px-4 py-2 border">අවු.0-04</th>
                        <th class="px-4 py-2 border">අවු.05-14</th>
                        <th class="px-4 py-2 border">අවු.15-59</th>
                        <th class="px-4 py-2 border">අවු.60-80</th>
                        <th class="px-4 py-2 border">අවු.80 ට වැඩි</th>
                        <th class="px-4 py-2 border">මුළු ජනගහනය</th>
                    </tr>
                    <tr class="pdFemale">
                        <td class="px-4 py-2 border">ස්ත්‍රී</td>
                        <x-editable-table-cell id="cell1"></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                    </tr>
                    <tr>
                        <td class="px-4 py-2 border">පුරුෂ</td>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                    </tr>
                </table>
                <x-primary-button id="btn-pup">Update</x-primary-button>
            </div>
        </div>
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 mb-5">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h2 class="text-xl font-semibold mb-4 dark:text-gray-200">2 ජන වර්ගය අනුව ජනගහනය </h2>

                <table class="shadow-md border rounded-lg w-full mb-5" border="1">
                    <tr>
                        <th></th>
                        <th class="px-4 py-2 border">සිංහල</th>
                        <th class="px-4 py-2 border">දෙමළ</th>
                        <th class="px-4 py-2 border">මුස්ලිම්</th>
                        <th class="px-4 py-2 border">මැලේ</th>
                        <th class="px-4 py-2 border">බර්ගර්</th>
                        <th class="px-4 py-2 border">වෙනත්</th>
                        <th class="px-4 py-2 border">එකතුව</th>
                    </tr>
                    <tr>
                        <td class="px-4 py-2 border">ස්ත්‍රී</td>
                        <x-editable-table-cell>0</x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                    </tr>
                    <tr>
                        <td class="px-4 py-2 border">පුරුෂ</td>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                    </tr>
                </table>
                <x-primary-button>Update</x-primary-button>
            </div>
        </div>
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 mb-5">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h2 class="text-xl font-semibold mb-4 dark:text-gray-200">3 ආගම අනුව ජනගහනය </h2>

                <table class="shadow-md border rounded-lg w-full mb-5" border="1">
                    <tr>
                        <th></th>
                        <th class="px-4 py-2 border">බෞද්ධ</th>
                        <th class="px-4 py-2 border">හින්දු</th>
                        <th class="px-4 py-2 border">ඉස්ලාම්</th>
                        <th class="px-4 py-2 border">කතෝලික</th>
                        <th class="px-4 py-2 border">වෙනත්</th>
                        <th class="px-4 py-2 border">මුළු ජනගහනය</th>

                    </tr>
                    <tr>
                        <td class="px-4 py-2 border">ස්ත්‍රී</td>
                        <x-editable-table-cell>0</x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>

                    </tr>
                    <tr>
                        <td class="px-4 py-2 border">පුරුෂ</td>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>
                        <x-editable-table-cell></x-editable-table-cell>

                    </tr>
                </table>
                <x-primary-button>Update</x-primary-button>
            </div>
        </div>






        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 mb-5">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">

                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <h2 class="text-xl font-semibold mb-4 dark:text-gray-200">4 විදේශගත ජනගහනය</h2>
                    <form id="foreignSourceForm"
                        class="max-w-2xl mx-auto p-8 bg-white dark:bg-gray-800 shadow-md rounded-md text-left">

                            @csrf

                            <div class="mb-6">
                                <label for="total_population"
                                    class="block font-medium text-sm text-gray-700 dark:text-gray-300">මුළු ජනගහනය
                                    :</label>
                                <input type="text" id="total_population" name="total_population" required
                                    class="mt-1 block w-80 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>

                            <div class="mb-6">
                                <label for="foreign_population"
                                    class="block font-medium text-sm text-gray-700 dark:text-gray-300">විදේශගත මුළු
                                    පුද්ගලයින් ගණන
                                    :</label>
                                <input type="text" id="foreign_population" name="foreign_population" required
                                    class="mt-1 block w-80 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>



                            <br>
                            <button type="submit"
                                class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                                Insert
                            </button>

                    </form>
                </div>


                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <table class="shadow-md border rounded-lg w-full">
                        <thead>
                            <tr>
                                <th class="px-4 py-2 border">ID</th>
                                <th class="px-4 py-2 border">මුළු ජනගහනය</th>
                                <th class="px-4 py-2 border">විදේශගත මුළු
                                    පුද්ගලයින් ගණන</th>
                            </tr>
                        </thead>
                        <tbody class="text-center" id="populationSourceTableBody">
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 mb-5">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">

                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <h2 class="text-xl font-semibold mb-4 dark:text-gray-200">5 මුළු පවුල් සංඛ්‍යාව/ගෘහ කාණ්ඩ සංඛ්‍යාව
                    </h2>
                    <form id="familySourceForm"
                        class="max-w-2xl mx-auto p-8 bg-white dark:bg-gray-800 shadow-md rounded-md text-left">
                        
                            @csrf

                            <div class="mb-6">
                                <label for="family_count"
                                    class="block font-medium text-sm text-gray-700 dark:text-gray-300">මුළු පවුල්
                                    සංඛ්‍යාව/ගෘහ කාණ්ඩ සංඛ්‍යාව
                                    :</label>
                                <input type="text" id="family_count" name="family_count" required
                                    class="mt-1 block w-80 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>

                            <div class="mb-6">
                                <label for="home_headed_count"
                                    class="block font-medium text-sm text-gray-700 dark:text-gray-300">කාන්තා ගෘහමූලික
                                    පවුල් සංඛ්‍යාව/ගෘහ කාණ්ඩ සංඛ්‍යාව
                                    :</label>
                                <input type="text" id="home_headed_count" name="home_headed_count" required
                                    class="mt-1 block w-80 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>



                            <br>
                            <button type="submit"
                                class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                                Insert
                            </button>

                    </form>
                </div>


                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <table class="shadow-md border rounded-lg w-full">
                        <thead>
                            <tr>
                                <th class="px-4 py-2 border">ID</th>
                                <th class="px-4 py-2 border">මුළු පවුල් සංඛ්‍යාව/ගෘහ කාණ්ඩ සංඛ්‍යාව</th>
                                <th class="px-4 py-2 border">කාන්තා ගෘහමූලික පවුල් සංඛ්‍යාව/ගෘහ කාණ්ඩ සංඛ්‍යාව</th>
                            </tr>
                        </thead>
                        <tbody class="text-center" id="homeSourceTableBody">
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
    <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 mb-5">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-4 dark:text-gray-200">6 විශේෂ අවශ්‍යතා සහිත පුද්ගලයින් සංඛ්‍යාව </h2>

            <table class="shadow-md border rounded-lg w-full mb-5" border="1">
                <tr>
                    <th></th>
                    <th class="px-4 py-2 border">අවු 18 ට අඩු සංඛ්‍යාව</th>
                    <th class="px-4 py-2 border">වයස 18 ට වැඩි විශේෂ
                        අවශ්‍යතා සහිත පුද්ගලයින් ගණන</th>
                    <th class="px-4 py-2 border">මුළු විශේෂ අවශ්‍යතා සහිත
                        පුද්ගලයින්</th>


                </tr>
                <tr>
                    <td class="px-4 py-2 border">පිරිමි</td>
                    <x-editable-table-cell>0</x-editable-table-cell>
                    <x-editable-table-cell></x-editable-table-cell>
                    <x-editable-table-cell></x-editable-table-cell>

                </tr>
                <tr>
                    <td class="px-4 py-2 border">ගැහැණු</td>
                    <x-editable-table-cell></x-editable-table-cell>
                    <x-editable-table-cell></x-editable-table-cell>
                    <x-editable-table-cell></x-editable-table-cell>

                </tr>

                <tr>
                    <td class="px-4 py-2 border">එකතුව</td>
                    <x-editable-table-cell></x-editable-table-cell>
                    <x-editable-table-cell></x-editable-table-cell>
                    <x-editable-table-cell></x-editable-table-cell>

                </tr>
            </table>
            <x-primary-button>Update</x-primary-button>
        </div>
    </div>




    </div>
    </div>
</x-app-layout>
<x-footer />
